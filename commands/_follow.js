...
  return Api.sendMessage({
    text: "Please reply to a *channel post* using: /follow [number]",
    parse_mode: "Markdown",
    reply_to_message_id: request.message_id
  });
}

// Ensure it's a valid Follow task
if (
  request.reply_to_message.text &&
  !request.reply_to_message.text.includes("Follower")
) {
  return Api.sendMessage({
    text: "⚠️ This task is not a <b>(Follow)</b> task.",
    reply_to_message_id: request.message_id,
    parse_mode: "HTML"
  });
}
...
  return Api.sendMessage({
    text: "Usage: /follow [number], e.g. /follow 10",
    reply_to_message_id: request.message_id
  });
}
...
if (count <= 0) {
  return Api.sendMessage({
    text: "❌ Number of follows must be more than 0.",
    reply_to_message_id: request.message_id
  });
}
...
// Many follows are already registered for this message
let taskId = "follow_" + request.reply_to_message.message_id;
let registeredFollows = Bot.getProperty(taskId) || 0;

if (registeredFollows + count > totalAvailable) {
  return Api.sendMessage({
    text: `⚠️ Only ${totalAvailable - registeredFollows} follows left for this task.`,
    reply_to_message_id: request.message_id
  });
}

// Update registered count
Bot.setProperty(taskId, registeredFollows + count, "integer");
...
Api.sendMessage({
  text: `<b>✅ ${count} follows have been registered by ${Name}</b>.\n\nTask Link :- <a href="${taskLink}">Link</a>`,
  parse_mode: "HTML",
  reply_to_message_id: request.message_id
});
